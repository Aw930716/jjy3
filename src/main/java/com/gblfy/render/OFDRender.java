...
public class OFDRender {
    ...
    public static void convertPdfToOfd(InputStream input, OutputStream output) throws IOException {
        ...
        try (PDDocument doc = PDDocument.load(input)) {
            com.gblfy.render.OFDCreator ofdCreator = new com.gblfy.render.OFDCreator();
            for (int i = 0; i < doc.getNumberOfPages(); i++) {
                ofdCreator.addPage(i);
                PDRectangle cropBox = doc.getPage(i).getCropBox();
                float widthPt = cropBox.getWidth();
                float heightPt = cropBox.getHeight();
                com.gblfy.render.OFDPageDrawer ofdPageDrawer = new com.gblfy.render.OFDPageDrawer(i, doc.getPage(i), ofdCreator, 1 / POINTS_PER_MM);
